...
def main(df,obs,dfxy=None):
    print(df.shape)
    df = df.apply(zscore)
    for i,uo in enumerate(sorted(list(obs.loc[:,"slide_scene"].unique()))):
        print(uo)
        key = obs.loc[:,"slide_scene"] == uo
        cm = corMat(df.loc[key,:].apply(zscore))
        #showHeatmap(cm)
        #getThresh(cm,df,obs)
        getThresh(cm,df.loc[key,:],obs.loc[key,:],t2 = uo)
...
def getThresh(cm,df,obs,t2=''):
    ...
    for i,b in enumerate(df.columns):
        ...
        optim = torch.optim.SGD([thresh], lr=.001)
        ...
        for i in range(NITERS):
            ...
            NC = neg.sum()
            PC = pos.sum()
            #loss = (ABA + BEB+ (NC+PC)**2)/((ABB+BEA)*(NC*PC)**.1)
