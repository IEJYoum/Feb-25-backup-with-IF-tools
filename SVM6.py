...
def trainSVM(X,Y,clf=None,name='tempSVM',mode="svm"):
...
def useSVM(X,Y=None,name='tempSVM'):
    clf = pickle.load(open(name+".sav", 'rb'))
    pr = clf.predict(X)
    '''
...
def trainNN(X,Y,clf=None,cat="temp",mode='nn'):
    print("training",mode,"on ",cat)
    ...
    if isinstance(clf,type(None)):
        if mode == 'svm':
            ...
            clf = MLPClassifier(solver='adam', alpha=1e-5,
                                 max_iter=1000,activation = 'tanh',
                                random_state=1)#'lbfgs'hidden_layer_sizes=(100, 20),
    odf = pd.DataFrame(X,index=Y)
    odf.to_csv("temp.csv")
    model = clf.fit(X,Y)
    pickle.dump(clf,open(cat+"_"+mode+".sav",'wb'))
    print("applying training results...",X.shape)
    return(clf.predict(X),model)


def buildData(df,obs,dfxy,yind=None):
    if not yind:
        yind = pickYind(obs)
    elif type(yind) != int:
        yind = list(obs.columns).index(yind)
